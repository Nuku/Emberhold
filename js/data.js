...
const TRIALS = [
  ...
  { id: 'newLands', name: 'Trial of To New Lands', repeat: 0,
    ...
    req: () => era() >= 5 && bld('tradeBlimp') > 0 && wondersWithAnEnding() >= LANDINGS.filter(landing => !landing.postWaters).length },
];
...
const UPGRADES = [
  ...
  { id: 'echoesOfHarvest', name: 'Echoes of Harvest', max: Infinity, cost: level => 20 * Math.ceil(1.4 ** level),
    effect: '+1% Food production, per level',
    desc: 'old hands remember how to tend the fields' },
  { id: 'echoesOfTimber', name: 'Echoes of Timber', max: Infinity, cost: level => 20 * Math.ceil(1.4 ** level),
    effect: '+1% Wood production, per level',
    desc: 'old hands remember how to find the strongest trees' },
  { id: 'echoesOfStone', name: 'Echoes of Stone', max: Infinity, cost: level => 20 * Math.ceil(1.4 ** level),
    effect: '+1% Stone production, per level',
    desc: 'old hands remember how to read the hills' },
  { id: 'bePrepared', name: 'Be Prepared', max: 1, costs: [400],
    req: () => trialCount('newLands') > 0,
    effect: '+2 Guard capacity, even before building a Barracks',
    desc: 'the first watch is already waiting when the settlement begins' },
  { id: 'runningHot', name: 'Running Hot', max: 1, costs: [800],
    req: () => trialCount('newLands') > 0,
    effect: '+50% Factory production, input consumption, and Power use',
    desc: 'the engines run harder, and every part of the line follows' },
...
const WONDERS = [
  ...
  { id: 'pallidExpanse', name: 'The White Engine', short: 'A buried machine that edits distance and bone',
    findCost: { survey: 520, aether: 140, steel: 420, machinery: 120 },
    findText: 'Across the Pallid Expanse, footprints lead toward one another from every direction. Beneath their intersection waits a white engine with no entrance, only a seam shaped like a horizon.',
    calamity: { resource: 'food', name: 'The Expanse edits the stores', values: [0.35, 0.8, 1.5, 2.6, 4.2], text: 'Food vanishes from locked rooms and reappears as frost beneath the expedition’s feet.' },
    sections: [['The Horizon Seam', 'The wall is farther away whenever someone approaches it.'], ['The Measured Snow', 'Every flake is a perfect map of somewhere else.'], ['The Cartilage Vault', 'White arches hang in rows, each grown from a different kind of bone.'], ['The Distance Loom', 'Threads of pale light stitch places together.'], ['The White Engine', 'A silent piston moves the world one fraction farther from itself.']],
    researches: [{ name: 'Surveyor’s Refusal', cost: { knowledge: 3600, tools: 90 }, effect: 'White Engine progress is 25% faster.', progress: 1.25 }, { name: 'Anchor the Near', cost: { knowledge: 7000, steel: 110, citizens: 1 }, effect: 'White Engine danger is reduced by 20%.', danger: 0.8 }, { name: 'The Rationed Horizon', cost: { knowledge: 12000, aether: 70, citizens: 2 }, effect: 'The store drain is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Plant Returning Stakes', cost: { steel: 180, tools: 80 }, effect: 'White Engine danger is reduced by 15%.', danger: 0.85 }, { name: 'Chart the Folded Miles', cost: { knowledge: 4200, machinery: 60 }, effect: 'White Engine progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The Engine can make every road shorter, every border thinner, and every home partly elsewhere.',
    aftermath: { restore: 'The White Engine settles the Expanse into dependable distances.', silence: 'The piston locks, and the world becomes difficult to alter.', become: 'Your leader enters the seam. Their shadow returns from several directions at once.' } },
  { id: 'glassMire', name: 'The Prism Womb', short: 'A wetland laboratory where reflections learn to hatch',
    findCost: { survey: 520, food: 2400, machinery: 130, starGlass: 90 },
    findText: 'The Glass Mire parts around a pool that reflects no one. Beneath its clear surface, translucent organs pulse around a stair descending into green light.',
    calamity: { resource: 'tools', name: 'The Mire softens the tools', values: [0.08, 0.18, 0.32, 0.52, 0.8], text: 'A damp forgets the difference between metal, wood, and flesh. Repairs multiply as tools lose their edges.' },
    sections: [['The Clear Placenta', 'A membrane ripples with images of people who have not arrived.'], ['The Embryo Ponds', 'Small shapes swim through nutrient glass; some have hands.'], ['The Pattern Nerves', 'Living crystal carries instructions and corrects mistakes by growing new ones.'], ['The Selection Choir', 'Voices compare every explorer to forms stored in the walls.'], ['The First Cell', 'A single cell divides only when someone promises it a future.']],
    researches: [{ name: 'Membrane Handling', cost: { knowledge: 3800, tools: 80 }, effect: 'Prism Womb progress is 25% faster.', progress: 1.25 }, { name: 'Sterile Names', cost: { knowledge: 7600, steel: 100, citizens: 1 }, effect: 'Prism Womb danger is reduced by 20%.', danger: 0.8 }, { name: 'The Heredity Ledger', cost: { knowledge: 13500, aether: 65, citizens: 2 }, effect: 'Tool corrosion is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Bottle the Clear Sap', cost: { food: 900, tools: 70 }, effect: 'Prism Womb danger is reduced by 15%.', danger: 0.85 }, { name: 'Sequence the First Cell', cost: { starGlass: 70, knowledge: 4800 }, effect: 'Prism Womb progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The First Cell has no ancestor and no purpose except becoming. The Ancients left both problems for you.',
    aftermath: { restore: 'The Prism Womb opens its pools to careful hands. New life begins beneath written limits.', silence: 'The cell returns to darkness; life remains gloriously beyond a committee.', become: 'Your leader dissolves into the First Cell. Every reflection blinks at once.' } },
  { id: 'hollowCanopy', name: 'The Breath Archive', short: 'A forest that stores the minds of extinct creatures',
    findCost: { survey: 520, wood: 2600, knowledge: 3600, heartwood: 100 },
    findText: 'The largest tree has no leaves and no visible hollow. At dusk it exhales the calls of animals that died before the Ancients learned to write.',
    calamity: { resource: 'wood', name: 'The Canopy grows around the settlement', values: [10, 23, 42, 68, 105], text: 'The Archive mistakes Emberhold for an unfinished specimen. Roots close around anything that stands still.' },
    sections: [['The Throat of the Tree', 'The trunk opens on a breath, its bark lined with worn teeth.'], ['The Nest of Names', 'Empty nests whisper their former occupants’ names.'], ['The Memory Sap', 'Amber holds the movements of vanished species.'], ['The Choir Galleries', 'Preserved calls overlap into a warning older than the forest.'], ['The Lungwood Core', 'The tree breathes, and something forgotten tries to return.']],
    researches: [{ name: 'Call Taxonomy', cost: { knowledge: 4000, wood: 300 }, effect: 'Breath Archive progress is 25% faster.', progress: 1.25 }, { name: 'Bark Boundary', cost: { knowledge: 7800, tools: 90, citizens: 1 }, effect: 'Breath Archive danger is reduced by 20%.', danger: 0.8 }, { name: 'The Pruning Treaty', cost: { knowledge: 14000, heartwood: 80, citizens: 2 }, effect: 'The timber loss is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Tie Down the Young Roots', cost: { wood: 1000, tools: 80 }, effect: 'Breath Archive danger is reduced by 15%.', danger: 0.85 }, { name: 'Record the Last Calls', cost: { knowledge: 5000, aether: 55 }, effect: 'Breath Archive progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The Lungwood can return what the world lost, but every resurrection needs a place to live.',
    aftermath: { restore: 'The Archive opens its memory to the canopy. Ancient calls return to the world.', silence: 'The final breath is held. No dead thing is asked to become useful again.', become: 'Your leader becomes the Archive’s newest remembered species.' } },
  { id: 'redChasm', name: 'The Red Equation', short: 'A living furnace that solves matter by consuming it',
    findCost: { survey: 520, stone: 2600, iron: 900, coal: 1500 },
    findText: 'The Red Chasm widens beneath the survey party, revealing a staircase cut into hot iron. Something below counts upward in impacts.',
    calamity: { resource: 'stone', name: 'The Chasm takes its braces', values: [4, 9, 17, 28, 44], text: 'The Red Equation tests the mountain by removing the answer. Stone disappears from supports before the cracks arrive.' },
    sections: [['The Counting Descent', 'Each stair bears a number that changes when spoken.'], ['The Ore Throat', 'Molten metal hardens into tools for creatures with too many joints.'], ['The Furnace Algebra', 'Failed proofs crawl away, hot and furious.'], ['The Pressure Choir', 'The mountain predicts the moment it will break.'], ['The Red Equation', 'A furnace waits for a material, a question, and a sacrifice.']],
    researches: [{ name: 'Thermal Notation', cost: { knowledge: 4200, coal: 500 }, effect: 'Red Equation progress is 25% faster.', progress: 1.25 }, { name: 'Brace Before Proof', cost: { knowledge: 8200, steel: 120, citizens: 1 }, effect: 'Red Equation danger is reduced by 20%.', danger: 0.8 }, { name: 'Finite Sacrifice', cost: { knowledge: 14500, machinery: 70, citizens: 2 }, effect: 'Stone demand is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Drive the Deep Bolts', cost: { iron: 500, steel: 100 }, effect: 'Red Equation danger is reduced by 15%.', danger: 0.85 }, { name: 'Copy the Furnace Proofs', cost: { knowledge: 5200, tools: 90 }, effect: 'Red Equation progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The Equation can answer any material question, provided Emberhold accepts what it counts as expendable.',
    aftermath: { restore: 'The Red Equation resumes its calculations. New alloys flow from the Chasm.', silence: 'The furnace cools; its questions no longer demand an answer.', become: 'Your leader steps into the equation. The mountain learns their name.' } },
  { id: 'drownedMoon', name: 'The Lunar Ossuary', short: 'A moon-sized relic buried beneath an inland sea',
    findCost: { survey: 520, knowledge: 4400, aether: 160, starGlass: 80 },
    findText: 'The Drowned Moon’s black tide exposes a stair of reflected stone. At its bottom, a pale sphere pulses beneath the water like a sleeping eye.',
    calamity: { resource: 'knowledge', name: 'The Moon drinks memory', values: [0.25, 0.58, 1.1, 1.9, 3.0], text: 'The relic pulls unfinished thoughts from every mind. Notes remain, but their authors forget why they mattered.' },
    sections: [['The Tidal Stair', 'The waterline climbs and falls with a heartbeat below the floor.'], ['The Crater Library', 'Books of lunar dust describe tides on worlds no astronomer can find.'], ['The Pale Marrow', 'White ribs surround a hollow where gravity points sideways.'], ['The Eclipse Bell', 'Each silent note removes a star from the reflected sky.'], ['The Sleeping Mare', 'A dark sea covers something that dreams in coastlines.']],
    researches: [{ name: 'Tide-Safe Notation', cost: { knowledge: 4600, tools: 80 }, effect: 'Lunar Ossuary progress is 25% faster.', progress: 1.25 }, { name: 'Remembering Ropes', cost: { knowledge: 9000, machinery: 70, citizens: 1 }, effect: 'Lunar Ossuary danger is reduced by 20%.', danger: 0.8 }, { name: 'The Mind Anchor', cost: { knowledge: 15000, aether: 90, citizens: 2 }, effect: 'Knowledge drain is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Weight the Moonstairs', cost: { steel: 140, starGlass: 60 }, effect: 'Lunar Ossuary danger is reduced by 15%.', danger: 0.85 }, { name: 'Chart the Missing Stars', cost: { knowledge: 5600, aether: 70 }, effect: 'Lunar Ossuary progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The Sleeping Mare could teach the sea a new shape, or teach Emberhold how small a world can be.',
    aftermath: { restore: 'The Lunar Ossuary rises beneath the tide, giving the Drowned Moon a calendar.', silence: 'The last bell is muffled. Memory remains your own.', become: 'Your leader descends into the Mare’s dream, and the tide rises toward no shore.' } },
  { id: 'boneOrchard', name: 'The Pale Genealogy', short: 'An orchard breeding bodies for purposes no one remembers',
    findCost: { survey: 520, food: 3000, wood: 1800, heartwood: 80 },
    findText: 'The Bone Orchard bears a new fruit each dawn. When one falls, its shell opens to reveal a complete skeleton with a seed where the heart should be.',
    calamity: { resource: 'food', name: 'The Orchard breeds from the stores', values: [0.45, 1.0, 1.8, 3.1, 5.0], text: 'Roots tunnel into Emberhold’s stores and return carrying teeth, shells, and half-finished appetites.' },
    sections: [['The Grafted Gate', 'Branches test each traveler for compatible roots.'], ['The Nursery Rows', 'Every pod contains a different answer to hunger.'], ['The Bone Trellis', 'Skeletons grow into branches and learn to bend with the wind.'], ['The Orchard Womb', 'Warm soil remembers every creature planted in it.'], ['The First Harvest', 'A perfect fruit waits, warm as a living animal.']],
    researches: [{ name: 'Seed-Bone Index', cost: { knowledge: 4400, food: 700 }, effect: 'Pale Genealogy progress is 25% faster.', progress: 1.25 }, { name: 'Gentle Grafting', cost: { knowledge: 8600, tools: 100, citizens: 1 }, effect: 'Pale Genealogy danger is reduced by 20%.', danger: 0.8 }, { name: 'The Hunger Boundary', cost: { knowledge: 14500, heartwood: 70, citizens: 2 }, effect: 'Food drain is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Fence the Hungry Roots', cost: { wood: 900, tools: 90 }, effect: 'Pale Genealogy danger is reduced by 15%.', danger: 0.85 }, { name: 'Taste the Safe Fruit', cost: { food: 1200, knowledge: 5000 }, effect: 'Pale Genealogy progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The First Harvest contains a body that could be grown for any purpose. The Orchard is waiting to hear yours.',
    aftermath: { restore: 'The Pale Genealogy flowers. New animals and symbioses enter the world beneath careful names.', silence: 'The perfect fruit rots. The Orchard returns to ordinary hunger and death.', become: 'Your leader is planted beneath the Harvest. By morning, the tree has learned to walk.' } },
  { id: 'blackTidelands', name: 'The Inkwell Leviathan', short: 'A drowned archive carried inside a living sea-beast',
    findCost: { survey: 520, food: 2800, steel: 300, oil: 700 },
    findText: 'The Black Tidelands leave a single enormous footprint. At low tide, it fills with ink and writes a route toward the sea.',
    calamity: { resource: 'oil', name: 'The tide swallows the fuel', values: [0.18, 0.42, 0.78, 1.35, 2.2], text: 'The Leviathan turns in its sleep. Black water carries Emberhold’s fuel toward the horizon.' },
    sections: [['The Footprint Harbor', 'A port has grown in the creature’s track for ships not yet built.'], ['The Ink Gills', 'Every exhalation releases a page.'], ['The Cartographer’s Gut', 'Maps line an organ that digests and rewrites them.'], ['The Salt Memory', 'Black brine replays every voyage the Leviathan swallowed.'], ['The Archive Heart', 'A central book is closed around one unspent instruction.']],
    researches: [{ name: 'Brine Cartography', cost: { knowledge: 4200, oil: 250 }, effect: 'Inkwell Leviathan progress is 25% faster.', progress: 1.25 }, { name: 'Sealed Breath', cost: { knowledge: 8200, steel: 100, citizens: 1 }, effect: 'Inkwell Leviathan danger is reduced by 20%.', danger: 0.8 }, { name: 'The Dry Harbor', cost: { knowledge: 14000, machinery: 70, citizens: 2 }, effect: 'Fuel loss is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Raise the Ink Buoys', cost: { oil: 300, steel: 120 }, effect: 'Inkwell Leviathan danger is reduced by 15%.', danger: 0.85 }, { name: 'Read the Swallowed Routes', cost: { knowledge: 5000, aether: 60 }, effect: 'Inkwell Leviathan progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The Leviathan’s instruction could open every sea, but it may also teach the sea how to open Emberhold.',
    aftermath: { restore: 'The Leviathan wakes and carries its archive along the coast.', silence: 'The great creature sinks, and its pages dissolve.', become: 'Your leader reads the book aloud. The Leviathan learns the shape of a human face.' } },
  { id: 'singingCrater', name: 'The Resonant Maw', short: 'A crater that sings instructions into living stone',
    findCost: { survey: 520, aether: 180, steel: 360, machinery: 100 },
    findText: 'The Singing Crater falls silent when approached. At its center, teeth rise from the rock and wait for Emberhold to make the first sound.',
    calamity: { resource: 'steel', name: 'The song unbuilds the steel', values: [0.12, 0.28, 0.52, 0.9, 1.5], text: 'The Maw finds the pitch inside forged metal. Hinges loosen and blades remember being ore.' },
    sections: [['The Quiet Rim', 'Silence presses against the ears; footfalls land before feet move.'], ['The Tuning Scree', 'Loose stones hum in scales that make teeth ache.'], ['The Choir Shafts', 'Voices below know the expedition’s next sentence.'], ['The Harmonic Foundry', 'Stone flows into instruments for throats, wings, and things without mouths.'], ['The Resonant Maw', 'The Ancient voice waits for a singer willing to answer.']],
    researches: [{ name: 'Interval Discipline', cost: { knowledge: 4800, tools: 90 }, effect: 'Resonant Maw progress is 25% faster.', progress: 1.25 }, { name: 'Mute the Harmonics', cost: { knowledge: 9200, steel: 110, citizens: 1 }, effect: 'Resonant Maw danger is reduced by 20%.', danger: 0.8 }, { name: 'The Tempered Answer', cost: { knowledge: 15500, aether: 100, citizens: 2 }, effect: 'Steel loss is reduced by 30%.', calamity: 0.7 }],
    expeditions: [{ name: 'Lay Felt in the Shafts', cost: { fur: 500, tools: 90 }, effect: 'Resonant Maw danger is reduced by 15%.', danger: 0.85 }, { name: 'Transcribe the Crater Song', cost: { knowledge: 5800, aether: 80 }, effect: 'Resonant Maw progress is 20% faster.', progress: 1.2 }],
    decisionText: 'The Maw can teach stone to sing, steel to remember, and a people to become one instrument.',
    aftermath: { restore: 'The Resonant Maw finds a stable chord and becomes a warning beacon.', silence: 'The answer is never sung. The Crater returns to wind and stone.', become: 'Your leader gives the Maw a voice. Every malformed creature turns toward the note.' } },
];
