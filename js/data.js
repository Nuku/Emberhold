...
const RESOURCES = [
  ...
  { id: 'aluminum', name: 'Aluminum', note: 'surprisingly light metal, refined for practical projects' },
...
const JOBS = {
  ...
  aluminumminer: { name: 'Aluminum Worker', res: 'aluminum', base: 0.05, mining: true, poweredBuilding: 'aluminumWorks', desc: 'refines the Ancients\' surprisingly light metal for practical projects',
                   max: () => bld('aluminumWorks') * 2,
                   unlock: () => bld('aluminumWorks') > 0 },
...
const BUILDINGS = [
  ...
  { id: 'aluminumWorks', name: 'Sky Metal Forge', max: Infinity, scale: 2.0,
    cost: { steel: 180, machinery: 40, goods: 60, copper: 120 },
    effect: () => '+2 Aluminum Worker capacity; +150 Aluminum storage; powered operation or 5 Coal/s',
    req: () => tech('aluminum'), desc: 'small projects in a surprisingly light metal' },
...
const TECHS = [
  ...
  { id: 'commonality', name: 'Commonality', cost: 2400,
    desc: 'Conquest is easy to name; belonging must be built after the smoke clears. Unlocks the Commonality government, returns five Guards from occupation duty for each conquered lineage, ends conquered-realm morale pressure, and strengthens ally income.',
...
const UPGRADES = [
  ...
  { id: 'fitTogether', name: 'Fit Together', max: 2, cost: level => level === 0 ? 500 : 1000,
    effect: 'building price-growth rates decrease by 1 percentage point, per level, to a 25% minimum',
    desc: 'We need more, but we have less. This is not a new problem, but it will require new answers.' },
  { id: 'wonderousAdvance', name: 'Wonderous Advance', max: 1, costs: [10000],
    req: () => wondersWithAnEnding() > 0,
    effect: 'Wonder obstacles are automatically added to the front of the construction queue',
    desc: 'The path ahead is known before it is reached. The queue makes room for what the Wonder demands.' },
  ...
  { id: 'knownTerrain', name: 'Known Terrain', max: 1, costs: [75],
    effect: 'always have the benefit of the Understanding Home research',
    desc: 'Some write down what the world has left. Take some time when migrating, and know where you go.' },
  { id: 'tillItIsComplete', name: 'Till It Is Complete', max: 1, costs: [100],
    req: () => upg('knownTask') > 0,
    effect: 'multi-part queued items take every affordable full step at once',
    desc: 'one hand over the next. The ancients achieved amazing things this way. Our methods are a dim echo of theirs' },
  { id: 'dividedAttention', name: 'Divided Attention', max: 1, costs: [200],
    effect: 'select two factory outputs; assigned factories and Tinkerers split their work between them',
    desc: 'As we advance, more things demand our attention. We cannot simple delay the second in favor of the first. Let us use both hands at once.' },
  { id: 'idleHands', name: 'Idle Hands', max: 1, costs: [250],
    effect: 'unassigned villagers produce Food as Foragers',
    desc: 'There is always work to be done. Don\'t stand around. If nothing comes to mind, help in the fields. We all need to eat.' },
  ...
  { id: 'adaptableGovernance', name: 'Adaptable Governance', max: 1, costs: [20],
    effect: 'government-swap cooldown is halved',
    desc: 'a living government knows when to change' },
